    ...
    >
      ...
      >
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 2 }}>
          ...
          <Button 
            variant="outlined" 
            onClick={() => router.push('/promedina/fwd/view')} 
            startIcon={<ArrowBack />}
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 3, border: `1px solid ${BORDER_GREEN}` }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr' }, gap: 2 }}>
            <Box>
              ...
              <Typography variant="body1" sx={{ fontWeight: 600 }}>{analysis.name || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary">Amostras</Typography>
              <Typography variant="body1" sx={{ fontWeight: 600 }}>{analysis.samples?.length || 0}</Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary">Status</Typography>
              <Chip 
                label={analysis.status === 'completed' ? 'Finalizada' : analysis.status === 'draft' ? 'Rascunho' : 'Ativa'}
                size="small"
            ...
            <Box>
              ...
              <Typography variant="body1" sx={{ fontWeight: 600 }}>{formatDate(analysis.createdAt)}</Typography>
            </Box>
            <Box>
        ...
        {procResult ? (
                ...
                <Table size="small">
                  <TableHead sx={{ backgroundColor: LIGHT_GREEN_BG }}>
                    <TableRow>
                      ...
                      {pos_sensores.map(d => (
                        <TableCell key={d} sx={{ fontWeight: 700, color: PRIMARY_GREEN }}>d{d}</TableCell>
                      ))}
                    </TableRow>
                  </TableHead>
                  <TableBody>
            ...
            <Paper variant="outlined" sx={{ p: 2, border: `1px solid ${BORDER_GREEN}` }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                <Chip 
                  ...
                  variant="outlined" color="success" sx={{ fontWeight: 600 }} 
                />
                <Chip 
                ...
                />
                <Chip 
                  label={`Média d0: ${(procResult.media_d0?.reduce((a: number, b: number) => a + b, 0) / procResult.media_d0?.length || 0).toFixed(1)} µm`} 
                  variant="outlined" sx={{ fontWeight: 600 }} 
                />
                <Chip 
                  label={`CV médio: ${(procResult.cv_d0?.reduce((a: number, b: number) => a + b, 0) / procResult.cv_d0?.length || 0).toFixed(1)}%`} 
                  variant="outlined" 
                ...
                <Chip 
                  ...
                  variant="outlined" color="error" sx={{ fontWeight: 600 }} 
                />
              </Box>
            </Paper>
          </>
          ...
          <Alert severity={analysis.samples?.length >= 5 ? 'info' : 'warning'}>
            {analysis.samples?.length >= 5 
              ...
              : `Análise possui apenas ${analysis.samples?.length || 0} amostras. Mínimo de 5 necessário para processamento.`}
          </Alert>
        )}
      </Paper>
    </Box>
