...
const ABCP_MaterialSelectionTable = ({ rows, columns, header }: Step2Props & { abcp: ABCP_SERVICE }) => {
        ...
        <DataGrid
          ...
          onRowSelectionModelChange={(rowSelection) => {
            if (rows.some((element) => element.type === 'cement')) {
              ...
              const cement = rowSelection.length > 0 ? rows[rowSelection[0]] : null;

              setData({ step: 1, key: 'cement', value: cement });
            } else {
              const aggregates = [];
