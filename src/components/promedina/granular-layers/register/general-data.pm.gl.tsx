...
const GranularLayers_step1 = ({ setNextDisabled }: EssayPageProps) => {
  ...
  // Helper para renderizar campos comuns - ANOTAÇÕES DE TIPO REMOVIDAS (inferidas do valor padrão)
  ...
  return (
    <>
      {/* CARD 1: IDENTIFICAÇÃO (existente, agora todo opcional) */}
      <FlexColumnBorder title="IDENTIFICAÇÃO" open={true} theme={'#07B811'}>
        ...
        >
          ...
          >
            ...
            {renderSelect('utilizadaLVECD', 'UTILIZADA NA CALIBRAÇÃO DO LVECD', generalData.utilizadaLVECD, simNaoOptions)}
      ...
      <FlexColumnBorder title="PREPARO DO PAVIMENTO" open={true} theme={'#07B811'}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '5px 20px', '& > *': { width: 'calc(50% - 10px)' } }}>
          {renderTextField('ir1PreReabilitacao', 'IR1 (m/km) Pré-Reabilitação', generalData.ir1PreReabilitacao, 'number')}
          ...
          {generalData.fresagem === 'Sim' &&
            renderTextField('espessuraFresagem', 'Espessura Fresada (mm)', generalData.espessuraFresagem, 'number')}
