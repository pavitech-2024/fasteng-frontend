...
const ShapeIndex_GeneralData = ({
  nextDisabled,
  setNextDisabled,
  shapeIndex,
}: EssayPageProps & { shapeIndex: SHAPEINDEX_SERVICE }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [materials, setMaterials] = useState<AsphaltMaterial[]>([]);
  const { user } = useAuth();
  const { generalData, setData } = useShapeIndexStore();
...
export default ShapeIndex_GeneralData;
