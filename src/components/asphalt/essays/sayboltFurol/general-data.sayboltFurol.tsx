...
const SayboltFurol_GeneralData = ({
  nextDisabled,
  setNextDisabled,
  sayboltFurol,
}: EssayPageProps & { sayboltFurol: SayboltFurol_SERVICE }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [materials, setMaterials] = useState<AsphaltMaterial[]>([]);
  const { user } = useAuth();
  const { generalData, setData } = useSayboltFurolStore();

  useEffect(() => {
    toast.promise(
      async () => {
        const materials = await sayboltFurol.getmaterialsByUserId(user._id);
...
export default SayboltFurol_GeneralData;
