...
const FlashPoint_GeneralData = ({
  nextDisabled,
  setNextDisabled,
  flashPoint,
}: EssayPageProps & { flashPoint: FLASHPOINT_SERVICE }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [materials, setMaterials] = useState<AsphaltMaterial[]>([]);
  const { user } = useAuth();
  const { generalData, setData } = useFlashPointStore();
...
export default FlashPoint_GeneralData;
