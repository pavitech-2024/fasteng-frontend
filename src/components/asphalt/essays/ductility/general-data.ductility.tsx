...
const Ductility_GeneralData = ({
  nextDisabled,
  setNextDisabled,
  ductility,
}: EssayPageProps & { ductility: DUCTILITY_SERVICE }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [materials, setMaterials] = useState<AsphaltMaterial[]>([]);
  const { user } = useAuth();
  const { generalData, setData } = useDuctilityStore();

  useEffect(() => {
    toast.promise(
      async () => {
        const data = await ductility.getmaterialsByUserId(user._id);
...
export default Ductility_GeneralData;
