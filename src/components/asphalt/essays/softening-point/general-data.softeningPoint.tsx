...
const SofteningPoint_GeneralData = ({
  ...
  setNextDisabled,
  softeningPoint,
}: EssayPageProps & { softeningPoint: SofteningPoint_SERVICE }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [materials, setMaterials] = useState<AsphaltMaterial[]>([]);
  const { user } = useAuth();
  const { generalData, setData } = useSofteningPointStore();

  useEffect(() => {
    toast.promise(
      async () => {
        const data = await softeningPoint.getmaterialsByUserId(user._id);
...
export default SofteningPoint_GeneralData;
