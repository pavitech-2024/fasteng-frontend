// components/organisms/essay-modal/hooks/useEssayModalData.ts
...
export const useEssayModalData = (essay: GranulometryEssay) => {
  const modalData = useMemo(() => ({
    container_other_data: [
      { label: t('granulometry-asphalt.accumulated-retained'), value: essay.results.accumulated_retained, unity: '%' },
      ...
      { label: t('asphalt.essays.granulometry.results.nominalDiammeter'), value: essay.results.nominal_diameter, unity: 'mm' },
      { label: t('asphalt.essays.granulometry.results.finenessModule'), value: essay.results.fineness_module, unity: '%' },
  ...
  const modalRows = useMemo(() => 
    essay.step2Data.table_data.map((value, index) => ({
      ...
      passant: essay.results.passant[index][1],
      retained_porcentage: essay.results.retained_porcentage[index][1],
      retained: value.retained,
      accumulated_retained: essay.results.accumulated_retained[index][1],
    })), [essay]
  );

  const modalColumns: GridColDef[] = useMemo(() => [
    { field: 'sieve', headerName: t('granulometry-asphalt.sieves'), valueFormatter: ({ value }) => `${value}` },
    { field: 'passant_porcentage', headerName: t('granulometry-asphalt.passant') + ' (%)', valueFormatter: ({ value }) => `${value}` },
    { field: 'passant', headerName: t('granulometry-asphalt.passant') + ' (g)', valueFormatter: ({ value }) => `${value}` },
    { field: 'retained_porcentage', headerName: t('granulometry-asphalt.retained') + ' (%)', valueFormatter: ({ value }) => `${value}` },
    { field: 'retained', headerName: t('granulometry-asphalt.retained') + ' (g)', valueFormatter: ({ value }) => `${value}` },
    { field: 'accumulated_retained', headerName: t('granulometry-asphalt.accumulated-retained') + ' (%)', valueFormatter: ({ value }) => `${value}` },
  ], []);
