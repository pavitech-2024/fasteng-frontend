...
const Angularity_GeneralData = ({
  nextDisabled,
  setNextDisabled,
  angularity,
}: EssayPageProps & { angularity: ANGULARITY_SERVICE }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [materials, setMaterials] = useState<AsphaltMaterial[]>([]);
  const { user } = useAuth();
  const { generalData, setData } = useAngularityStore();
...
export default Angularity_GeneralData;
