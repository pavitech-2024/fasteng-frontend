...
const FWD_step3 = ({ setNextDisabled }: EssayPageProps) => {
  ...
  const baciaChartData =
    procResult && procResult.subtrechos && procResult.subtrechos.length > 0
      ? {
    ...
    <>
        ...
        <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {!selectedAnalysis ? (
            ...
            <Alert severity="warning" sx={{ width: '100%' }}>
              Análise <strong>{selectedAnalysis.name}</strong> possui apenas {selectedAnalysis.samples?.length || 0}{' '}
              amostras. Mínimo de 5 amostras necessário para processamento.
            </Alert>
          ) : (
            <>
              <Box sx={{ width: '100%', display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
                ...
                <Chip
                  ...
                  sx={{ fontWeight: 600 }}
                />
                {procResult && <Chip label="Processada" color="info" variant="outlined" sx={{ fontWeight: 600 }} />}
              </Box>
              ...
              <Box sx={{ width: '100%', mt: 1 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Tooltip
                    title="Os subtrechos são definidos por quebras no Coeficiente de Variação (CV > 30%) ou comprimento máximo de 2000m. O último subtrecho não tem restrição de comprimento mínimo."
                    arrow
                  >
      ...
      {procResult && procResult.ordered && procResult.ordered.length > 0 && (
        <>
          ...
          <FlexColumnBorder
            title={`SUBTRECHOS HOMOGÊNEOS (${procResult.subtrechos?.length || 0})`}
            open={true}
            theme={'#07B811'}
            ...
            <Box sx={{ width: '100%' }}>
              <TableContainer component={Paper} sx={{ border: '1px solid #07B811', borderRadius: 1 }}>
                <Table size="small">
                  <TableHead sx={{ backgroundColor: '#f0fff0' }}>
                    <TableRow>
                      <TableCell sx={{ fontWeight: 700, color: '#07B811', whiteSpace: 'nowrap' }}>
                        INÍCIO (EST.)
                      </TableCell>
                    ...
                    {procResult.subtrechos?.map((sub, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {pos_sensores.map((d) => (
                            <TableCell key={d}>{sub[`d${d}` as keyof typeof sub]}</TableCell>
                          ))}
                        </TableRow>
                      );
                    })}
              ...
              {procResult.subtrechos?.some((s, i) => {
                ...
                <Alert severity="warning" sx={{ mt: 2 }}>
                  Alguns subtrechos estão abaixo de 200m. Apenas o último subtrecho pode ter comprimento inferior ao
                  mínimo.
                </Alert>
              )}
            </Box>
          ...
          {baciaChartData && baciaChartData.datasets && baciaChartData.datasets.length > 0 && (
            <FlexColumnBorder
              title="BACIAS DE DEFLEXÃO CARACTERÍSTICAS"
              open={true}
              theme={'#07B811'}
              sx_title={{ whiteSpace: 'wrap' }}
            >
          ...
          {/* ESTATÍSTICAS */}
          <FlexColumnBorder
            title="ESTATÍSTICAS DO PROCESSAMENTO"
            open={true}
            theme={'#07B811'}
            sx_title={{ whiteSpace: 'wrap' }}
          >
            <Box sx={{ width: '100%', display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              ...
              <Chip
                label={`Média d0: ${(
                  procResult.media_d0?.reduce((a, b) => a + b, 0) / procResult.media_d0?.length || 0
                ).toFixed(1)} µm`}
                variant="outlined"
                sx={{ fontWeight: 600 }}
              />
              <Chip
                label={`CV médio: ${(
                  procResult.cv_d0?.reduce((a, b) => a + b, 0) / procResult.cv_d0?.length || 0
                ).toFixed(1)}%`}
                variant="outlined"
        ...
        <FlexColumnBorder title="INSTRUÇÕES" open={true} theme={'#07B811'} sx_title={{ whiteSpace: 'wrap' }}>
          <Box sx={{ width: '100%', textAlign: 'center', py: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Clique em &quot;PROCESSAR ANÁLISE&quot; para gerar os subtrechos homogêneos e visualizar os gráficos de
              deflexão.
            </Typography>
          </Box>
        </FlexColumnBorder>
