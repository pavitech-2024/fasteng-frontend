...
const ElongatedParticles_GeneralData = ({
  nextDisabled,
  setNextDisabled,
  elongatedParticles,
}: EssayPageProps & { elongatedParticles: ELONGATEDPARTICLES_SERVICE }) => {
  const [loading, setLoading] = useState<boolean>(true);
  const [materials, setMaterials] = useState<AsphaltMaterial[]>([]);
  const { user } = useAuth();
  const { generalData, setData } = useElongatedParticlesStore();
...
export default ElongatedParticles_GeneralData;
