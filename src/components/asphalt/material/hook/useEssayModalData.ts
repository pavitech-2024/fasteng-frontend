...
export const useEssayModalData = (essay: any) => {
  ...
  const essayType = useMemo(() => {
    ...
    if (results?.machiningTemperatureRange !== undefined || results?.curvePoints !== undefined) {
  ...
  const modalExperimentResumeData: ExperimentResumeData = useMemo(
    () => ({
      ...
      author:
        essay.generalData?.userId || essay.generalData?.calculist || essay.generalData?.operator || 'Não informado',
    ...
    switch (essayType) {
      ...
      case ESSAY_TYPES.LOS_ANGELES_ABRASION:
        return {
          results: [
            {
              label: 'Perda por abrasão',
              value:
                results?.losAngelesAbrasion?.toFixed(2) ||
                results?.abrasion_loss?.toFixed(2) ||
                results?.wear_loss?.toFixed(2),
              unit: '%',
            },
            {
              label: 'Massa inicial',
              value: essay.abrasionCalc?.initialMass || essay.step2Data?.initial_mass,
              unit: 'g',
            },
      ...
      case ESSAY_TYPES.ELONGATED_PARTICLES:
        return {
          results: [{ label: 'Partículas alongadas', value: results?.elongated_percentage?.toFixed(2), unit: '%' }],
        };

      case ESSAY_TYPES.SHAPE_INDEX:
        return {
          results: [{ label: 'Índice de Forma', value: results?.shape_index?.toFixed(2), unit: '%' }],
        };

      case ESSAY_TYPES.SAND_EQUIVALENT:
        return {
          results: [{ label: 'Equivalente de Areia', value: results?.sand_equivalent?.toFixed(2), unit: '%' }],
        };

      case ESSAY_TYPES.ANGULARITY:
        return {
          results: [{ label: 'Angularidade', value: results?.angularity?.toFixed(2), unit: '%' }],
        };
      ...
      case ESSAY_TYPES.FLASH_POINT:
        return {
          results: [{ label: 'Temperatura de Ignição', value: results?.temperature, unit: '°C' }],
        };
      ...
      case ESSAY_TYPES.VISCOSITY_ROTATIONAL:
        return {
          results: [
            {
              label: 'Temp. Usinagem (mín)',
              value: results?.machiningTemperatureRange?.lower?.toFixed(2),
              unit: '°C',
            },
            {
              label: 'Temp. Usinagem (máx)',
              value: results?.machiningTemperatureRange?.higher?.toFixed(2),
              unit: '°C',
            },
            {
      ...
      case ESSAY_TYPES.ELASTIC_RECOVERY:
        return {
          results: [{ label: 'Recuperação Elástica', value: results?.elastic_recovery?.toFixed(2), unit: '%' }],
        };
